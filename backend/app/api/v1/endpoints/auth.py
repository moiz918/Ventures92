...
def login(
    ...
    if user is None:
        # Run a dummy verify so the response time roughly matches the real path.
...
def forgot_password(
    ...
    if user is not None and user.is_active:
        ...
        # ── Email abstraction stub ─────────────────────────────────────────
        # In production replace this with your email service (SES, SendGrid,
        # Resend, etc.).  For now we log the link to backend stdout.
        logger.warning(
...
def reset_password(
    ...
    # Apply new password and invalidate the token + lockout state.
